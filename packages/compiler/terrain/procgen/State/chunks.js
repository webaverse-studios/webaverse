...
export default class Chunks {
  ...
  update() {
    ...
    const playerChunkKey = `${Math.round(player.position.x / this.minSize * 2 + 0.5)}${Math.round(player.position.z / this.minSize * 2 + 0.5)}`;
  ...
  getMainChunksCoordinates() {
    const player = this.state.player;
    const currentX = Math.round(player.position.x / this.maxSize);
    const currentZ = Math.round(player.position.z / this.maxSize);
  ...
  underSplitDistance(size, chunkX, chunkY) {
    const player = this.state.player;
    const distance = Math.hypot(player.position.x - chunkX, player.position.z - chunkY);
