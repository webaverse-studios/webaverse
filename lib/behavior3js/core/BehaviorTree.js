...
export default class BehaviorTree {
  ...
  tick(target, blackboard) {
    ...
    //     // start -= 1; // Fix bug that, after high priority MemSequence overwrite/halt low priority MemSequence, low priority MemSequence not be reset ( open() not run, runningChild not reset to 0 ).
    //     // The above `start = i + 1;` still needed, to handle other ordinary cases.
    //     break;
    //   }
    // }
    ...
    blackboard.set('prevTick', tick, tick.tree.id);
